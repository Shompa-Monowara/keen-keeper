...
const Timeline = () => {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "Video": return <FiVideo className="text-xl" />; // ✅ Fixed Typo
      default: return null;
    }
  };
          ...
          <div className="mb-8">
            <div className="dropdown w-full"> 
              ...
              <ul tabIndex={0} className="dropdown-content z-[10] menu p-0 py-1 shadow-2xl bg-base-100 rounded-lg max-w-[320px] w-full mt-1 border border-gray-100">
                <li>
                  <button onClick={() => handleFilterChange("All")} className="flex items-center gap-3 py-2 px-4 text-[#555] hover:bg-gray-100 rounded-none">
                    <div className="w-4 flex justify-center">{filter === "All" && <FaCheck className="text-[12px] text-gray-400" />}</div>
                    <span>Filter timeline</span>
                  </button>
                </li>
                {["Text", "Call", "Video"].map((item) => (
                  <li key={item}>
                    <button onClick={() => handleFilterChange(item)} className="flex items-center gap-3 py-2 px-4 text-[#1a1a1a] hover:bg-gray-100 rounded-none font-medium">
                      <div className="w-4 flex justify-center">{filter === item && <FaCheck className="text-[12px] text-[#1F2937]" />}</div>
                      <span>{item}</span>
                    </button>
                  </li>
                ))}
